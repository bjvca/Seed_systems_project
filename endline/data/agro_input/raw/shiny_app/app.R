...
reviews <- read.csv("endline_reviews_general.csv")[c("shop_ID","general_rating","location_rating","price_rating","quality_rating", "stock_rating","reputation_rating" ,"nr_reviews" )]

shops <- merge(shops, reviews,by.x="shop_ID", by.y="shop_ID")
#merge in ratings from latest data
shops$rating <- rowSums(shops[c("general_rating",                       
 "location_rating",                      
"price_rating",                         
"quality_rating",                       
"stock_rating",     
...
### just to rename
inter <- shops[c("shop_ID","catchID","general_rating","location_rating", "price_rating","quality_rating","stock_rating", "reputation_rating")] 
names(inter) <- c("shop_ID","catchID","general","location", "price","quality","stock", "reputation")

dot_frame <- melt(inter[c("shop_ID","catchID","general","location", "price","quality","stock", "reputation")], id.vars=c("shop_ID", "catchID"))
...
titlePanel("Uganda Agro-input Advisor"),
  sidebarPanel(
     #Outputs
