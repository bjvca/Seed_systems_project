...
balance_dealer <- c("maize.owner.agree.age"
                    ...
                    ,"maize.owner.agree.inspection.q115" #15
                    ,"reading"
                    ,"lot"
                    ,"refunds"
                    ,"gives_credit"
...
for (i in 1:length(balance_dealer)){
  formula1 <- as.formula(paste(balance_dealer[i],paste("training*clearing_demeaned*farmer_demeaned"),sep="~"))
...
for (i in 1:length(balance_dealer)){
  formula1 <- as.formula(paste(balance_dealer[i],paste("training_demeaned*clearing*farmer_demeaned"),sep="~"))
  ols <- lm(formula1, data=baseline_dealers)
  vcov_cluster <- vcovCR(ols,cluster=baseline_dealers$catchID,type="CR3")
...
for (i in 1:length(balance_dealer)){
  formula1 <- as.formula(paste(balance_dealer[i],paste("training_demeaned*clearing_demeaned*farmer"),sep="~"))
  ols <- lm(formula1, data=baseline_dealers)
  vcov_cluster <- vcovCR(ols,cluster=baseline_dealers$catchID,type="CR3")
...
for (i in 1:length(balance_farmer)){
  formula2 <- as.formula(paste(balance_farmer[i],paste("training*clearing_demeaned*farmer_demeaned"),sep="~")) #* because of interactions
...
for (i in 1:length(balance_farmer)){
  formula2 <- as.formula(paste(balance_farmer[i],paste("training_demeaned*clearing*farmer_demeaned"),sep="~")) #* because of interactions
  ols <- lm(formula2, data=baseline_farmers)
  #vcovCR for cluster-robust variance-covariance matrix
  vcov_cluster_catchID <- vcovCR(ols,cluster=baseline_farmers$catchID,type="CR0")
...
for (i in 1:length(balance_farmer)){
  formula2 <- as.formula(paste(balance_farmer[i],paste("training_demeaned*clearing_demeaned*farmer"),sep="~")) #* because of interactions
  ols <- lm(formula2, data=baseline_farmers)
  #vcovCR for cluster-robust variance-covariance matrix
  vcov_cluster_catchID <- vcovCR(ols,cluster=baseline_farmers$catchID,type="CR0")
...
for (i in 1:length(attrition_dealer)){
  formula1 <- as.formula(paste(attrition_dealer[i],paste("training*clearing_demeaned*farmer_demeaned"),sep="~"))
  ols <- lm(formula1, data=baseline_dealers)
  vcov_cluster <- vcovCR(ols,cluster=baseline_dealers$catchID,type="CR3")
...
for (i in 1:length(attrition_dealer)){
  formula1 <- as.formula(paste(attrition_dealer[i],paste("training_demeaned*clearing*farmer_demeaned"),sep="~"))
  ols <- lm(formula1, data=baseline_dealers)
  vcov_cluster <- vcovCR(ols,cluster=baseline_dealers$catchID,type="CR3")
...
for (i in 1:length(attrition_dealer)){
  formula1 <- as.formula(paste(attrition_dealer[i],paste("training_demeaned*clearing_demeaned*farmer"),sep="~"))
  ols <- lm(formula1, data=baseline_dealers)
  vcov_cluster <- vcovCR(ols,cluster=baseline_dealers$catchID,type="CR3")
...
save(df_ols_attritionD,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_attritionD.Rdata",sep="/"))
...
for (i in 1:length(attrition_farmer)){
  formula2 <- as.formula(paste(attrition_farmer[i],paste("training*clearing_demeaned*farmer_demeaned"),sep="~")) #* because of interactions
  ols <- lm(formula2, data=baseline_farmers)
  #vcovCR for cluster-robust variance-covariance matrix
  vcov_cluster_catchID <- vcovCR(ols,cluster=baseline_farmers$catchID,type="CR0")
...
for (i in 1:length(attrition_farmer)){
  formula2 <- as.formula(paste(attrition_farmer[i],paste("training_demeaned*clearing*farmer_demeaned"),sep="~")) #* because of interactions
  ols <- lm(formula2, data=baseline_farmers)
  #vcovCR for cluster-robust variance-covariance matrix
  vcov_cluster_catchID <- vcovCR(ols,cluster=baseline_farmers$catchID,type="CR0")
...
for (i in 1:length(attrition_farmer)){
  formula2 <- as.formula(paste(attrition_farmer[i],paste("training_demeaned*clearing_demeaned*farmer"),sep="~")) #* because of interactions
  ols <- lm(formula2, data=baseline_farmers)
  #vcovCR for cluster-robust variance-covariance matrix
  vcov_cluster_catchID <- vcovCR(ols,cluster=baseline_farmers$catchID,type="CR0")
...
#baseline_dealers_results_dealer_prim_base_save <- baseline_dealers[results_dealer_prim_base]

baseline_dealers[results_dealer_prim_base] <- lapply(baseline_dealers[results_dealer_prim_base],function(x)x - mean(x,na.rm = T))

#ATTENTION
#e.g. quantitysold is "lapply'd" (demeaned) like this 4 times
#so quantitysold is something else in every loop even though mid_quantitysold remains the same
#however, that does not change the results
#I tried and the results are robust to correcting for this
...
#baseline_dealers[results_dealer_prim_base] <- baseline_dealers_results_dealer_prim_base_save

save(df_means_D_prim,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_prim.Rdata",sep="/"))
...
save(df_ols_D_prim,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_prim.Rdata",sep="/"))
...
results_dealer_prim <- c("mid_quantitysold","mid_av_salesprices","mid_revenue"
                         ,"mid_maize.owner.agree.q7","index_practices_cap_mid"
...
df_means_D_prim_HET <- array(NA,dim=c(3,10))

for (i in 1:length(results_dealer_prim)){
  df_means_D_prim_HET[1,i] <- sum(baseline_dealers[results_dealer_prim[i]], na.rm=T)/(nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_prim[i]])))
  df_means_D_prim_HET[2,i] <- sqrt(var(baseline_dealers[results_dealer_prim[i]], na.rm=T))
  df_means_D_prim_HET[3,i] <- nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_prim[i]]))-sum(is.na(baseline_dealers[results_dealer_prim_base[i]]))+sum(is.na(baseline_dealers[results_dealer_prim[i]])&is.na(baseline_dealers[results_dealer_prim_base[i]]))}

#baseline_dealers[results_dealer_prim_base] <- baseline_dealers_results_dealer_prim_base_save

save(df_means_D_prim_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_prim_HET.Rdata",sep="/"))

###
#2#
###

df_ols_D_prim_HET <- array(NA,dim=c(3,3,10))
...
for (i in 1:length(results_dealer_prim)){
  ...
  df_ols_D_prim_HET[1,1,i] <- coef_test(ols, vcov_cluster)$beta[2]
  df_ols_D_prim_HET[2,1,i] <- coef_test(ols, vcov_cluster)$SE[2]
  df_ols_D_prim_HET[3,1,i] <- coef_test(ols, vcov_cluster)$p_Satt[2]}
...
for (i in 1:length(results_dealer_prim)){
  ...
  df_ols_D_prim_HET[1,2,i] <- coef_test(ols, vcov_cluster)$beta[3]
  df_ols_D_prim_HET[2,2,i] <- coef_test(ols, vcov_cluster)$SE[3]
  df_ols_D_prim_HET[3,2,i] <- coef_test(ols, vcov_cluster)$p_Satt[3]}
...
for (i in 1:length(results_dealer_prim)){
  ...
  #farmer video treatment at village/shop level so no clustering needed
  df_ols_D_prim_HET[1,3,i] <- summary(ols)$coefficients[4,1]
  df_ols_D_prim_HET[2,3,i] <- summary(ols)$coefficients[4,2]
  df_ols_D_prim_HET[3,3,i] <- summary(ols)$coefficients[4,4]}

save(df_ols_D_prim_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_prim_HET.Rdata",sep="/"))
...
results_dealer_sec_base <- c("maize.owner.agree.nr_var","maize.owner.agree.q19","maize.owner.agree.q44","maize.owner.agree.inspection.q121"
                             ...
                             ,"maize.owner.agree.skill.q104_bin","maize.owner.agree.skill.q105_bin"
                             ,"maize.owner.agree.skill.q106_bin","maize.owner.agree.skill.q107_bin"
                             ,"maize.owner.agree.skill.q108_bin"
                             ,"maize.owner.agree.skill.q109_bin","maize.owner.agree.skill.q111_bin"
                             ,"maize.owner.agree.skill.q112_bin","maize.owner.agree.skill.q113_bin")

df_means_D_sec <- array(NA,dim=c(3,20))
...
#BASELINE
for (i in 1:length(results_dealer_sec_base)){
  df_means_D_sec[1,i] <- sum(baseline_dealers[results_dealer_sec_base[i]], na.rm=T)/(nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_sec_base[i]])))
  df_means_D_sec[2,i] <- sqrt(var(baseline_dealers[results_dealer_sec_base[i]], na.rm=T))
  df_means_D_sec[3,i] <- nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_sec[i]]))-sum(is.na(baseline_dealers[results_dealer_sec_base[i]]))+sum(is.na(baseline_dealers[results_dealer_sec[i]])&is.na(baseline_dealers[results_dealer_sec_base[i]]))}

save(df_means_D_sec,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_sec.Rdata",sep="/"))
...
df_ols_D_sec <- array(NA,dim=c(3,3,20))
...
save(df_ols_D_sec,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_sec.Rdata",sep="/"))
...
save(df_means_D_secL10H_B,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_secL10H_B.Rdata",sep="/"))
...
save(df_ols_D_secL10H_B,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_secL10H_B.Rdata",sep="/"))
...
results_dealer_secL5 <- c("mid_maize.owner.agree.longe5.q46","mid_maize.owner.agree.longe5.q47","mid_maize.owner.agree.longe5.q49"
                            ...
                            ,"mid_maize.owner.agree.longe5.q55","index_overall_Longe5_mid","mid_revenue_longe5")
...
results_dealer_secL5_base <- c("maize.owner.agree.longe5.q46","maize.owner.agree.longe5.q47","maize.owner.agree.longe5.q49"
                                 ...
                                 ,"maize.owner.agree.longe5.q55","index_overall_Longe5_base","revenue_longe5")
...
save(df_means_D_secL5,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_secL5.Rdata",sep="/"))
...
results_dealer_secL5 <- c("mid_maize.owner.agree.longe5.q46","mid_maize.owner.agree.longe5.q47","mid_maize.owner.agree.longe5.q49"
                            ,"mid_maize.owner.agree.longe5.q50","mid_maize.owner.agree.longe5.q51","mid_maize.owner.agree.longe5.q52"
                            ,"mid_maize.owner.agree.longe5.q55","index_overall_Longe5_midT","mid_revenue_longe5")

results_dealer_secL5_base <- c("maize.owner.agree.longe5.q46","maize.owner.agree.longe5.q47","maize.owner.agree.longe5.q49"
                                 ,"maize.owner.agree.longe5.q50","maize.owner.agree.longe5.q51","maize.owner.agree.longe5.q52"
                                 ,"maize.owner.agree.longe5.q55","index_overall_Longe5_baseT","revenue_longe5")
...
results_dealer_secL5 <- c("mid_maize.owner.agree.longe5.q46","mid_maize.owner.agree.longe5.q47","mid_maize.owner.agree.longe5.q49"
                            ,"mid_maize.owner.agree.longe5.q50","mid_maize.owner.agree.longe5.q51","mid_maize.owner.agree.longe5.q52"
                            ,"mid_maize.owner.agree.longe5.q55","index_overall_Longe5_midC","mid_revenue_longe5")

results_dealer_secL5_base <- c("maize.owner.agree.longe5.q46","maize.owner.agree.longe5.q47","maize.owner.agree.longe5.q49"
                                 ,"maize.owner.agree.longe5.q50","maize.owner.agree.longe5.q51","maize.owner.agree.longe5.q52"
                                 ,"maize.owner.agree.longe5.q55","index_overall_Longe5_baseC","revenue_longe5")
...
results_dealer_secL5 <- c("mid_maize.owner.agree.longe5.q46","mid_maize.owner.agree.longe5.q47","mid_maize.owner.agree.longe5.q49"
                            ,"mid_maize.owner.agree.longe5.q50","mid_maize.owner.agree.longe5.q51","mid_maize.owner.agree.longe5.q52"
                            ,"mid_maize.owner.agree.longe5.q55","index_overall_Longe5_midF","mid_revenue_longe5")

results_dealer_secL5_base <- c("maize.owner.agree.longe5.q46","maize.owner.agree.longe5.q47","maize.owner.agree.longe5.q49"
                                 ,"maize.owner.agree.longe5.q50","maize.owner.agree.longe5.q51","maize.owner.agree.longe5.q52"
                                 ,"maize.owner.agree.longe5.q55","index_overall_Longe5_baseF","revenue_longe5")
...
save(df_ols_D_secL5,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_secL5.Rdata",sep="/"))
...
variables_overall_Longe5_mid <- cbind(baseline_dealers$mid_maize.owner.agree.longe5.q46,baseline_dealers$mid_maize.owner.agree.longe5.q47
                                      ...
                                      ,baseline_dealers$mid_maize.owner.agree.longe5.q55,baseline_dealers$mid_maize.owner.agree.longe5.q51
                                      ,baseline_dealers$mid_revenue_longe5) #x
variables_overall_Longe5_base <- cbind(baseline_dealers$maize.owner.agree.longe5.q46,baseline_dealers$maize.owner.agree.longe5.q47
...
index_overall_Longe5_base <- icwIndex(xmat=variables_overall_Longe5_base,revcols = c(1,4,5))
baseline_dealers$index_overall_Longe5_base <- index_overall_Longe5_base$index
...
save(df_means_D_secL5_B,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_secL5_B.Rdata",sep="/"))
...
save(df_ols_D_secL5_B,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_secL5_B.Rdata",sep="/"))
...
variables_overall_off_mid <- cbind(baseline_dealers$mid_maize.owner.agree.inspection.q114,baseline_dealers$mid_maize.owner.agree.inspection.q115
                                   ...
                                   ,baseline_dealers$mid_maize.owner.agree.temp.q81) #x
variables_overall_off_base <- cbind(baseline_dealers$maize.owner.agree.inspection.q114,baseline_dealers$maize.owner.agree.inspection.q115
                                    ,baseline_dealers$maize.owner.agree.inspection.q116,baseline_dealers$maize.owner.agree.inspection.q117
                                    ,baseline_dealers$maize.owner.agree.temp.q81)
...
baseline_dealers$owner.agree.q5 <- as.character(baseline_dealers$owner.agree.q5)

baseline_dealers$owner.agree.q5[baseline_dealers$owner.agree.q5=="n/a"] <- NA
baseline_dealers$owner.agree.q5[baseline_dealers$owner.agree.q5=="Yes"] <- 1
baseline_dealers$owner.agree.q5[baseline_dealers$owner.agree.q5=="No"] <- 0

baseline_dealers$owner.agree.q5 <- as.numeric(as.character(baseline_dealers$owner.agree.q5))
...
results_dealer_sec_off_base <- c("maize.owner.agree.inspection.q114","maize.owner.agree.inspection.q115","maize.owner.agree.inspection.q116","maize.owner.agree.inspection.q117"
                                 ...
                                 ,"index_overall_off_base","maize.owner.agree.q96")
...
save(df_means_D_sec_off,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_sec_off.Rdata",sep="/"))
...
results_dealer_sec_off <- c("mid_maize.owner.agree.inspection.q114","mid_maize.owner.agree.inspection.q115","mid_maize.owner.agree.inspection.q116","mid_maize.owner.agree.inspection.q117"
                            ...
                            ,"index_overall_off_midT","mid_maize.owner.agree.q96")

results_dealer_sec_off_base <- c("maize.owner.agree.inspection.q114","maize.owner.agree.inspection.q115","maize.owner.agree.inspection.q116","maize.owner.agree.inspection.q117"
                                 ,"maize.owner.agree.inspection.q118","maize.owner.agree.inspection.q119"
                                 ,"index_overall_off_baseT","maize.owner.agree.q96")
...
results_dealer_sec_off <- c("mid_maize.owner.agree.inspection.q114","mid_maize.owner.agree.inspection.q115","mid_maize.owner.agree.inspection.q116","mid_maize.owner.agree.inspection.q117"
                            ,"mid_maize.owner.agree.inspection.q118","mid_maize.owner.agree.inspection.q119"
                            ,"index_overall_off_midC","mid_maize.owner.agree.q96")

results_dealer_sec_off_base <- c("maize.owner.agree.inspection.q114","maize.owner.agree.inspection.q115","maize.owner.agree.inspection.q116","maize.owner.agree.inspection.q117"
                                 ,"maize.owner.agree.inspection.q118","maize.owner.agree.inspection.q119"
                                 ,"index_overall_off_baseC","maize.owner.agree.q96")
...
baseline_dealers$index_quality_alternative_baseF <- index_overall_off_base$index

results_dealer_sec_off <- c("mid_maize.owner.agree.inspection.q114","mid_maize.owner.agree.inspection.q115","mid_maize.owner.agree.inspection.q116","mid_maize.owner.agree.inspection.q117"
                            ,"mid_maize.owner.agree.inspection.q118","mid_maize.owner.agree.inspection.q119"
                            ,"index_overall_off_midF","mid_maize.owner.agree.q96")

results_dealer_sec_off_base <- c("maize.owner.agree.inspection.q114","maize.owner.agree.inspection.q115","maize.owner.agree.inspection.q116","maize.owner.agree.inspection.q117"
                                 ,"maize.owner.agree.inspection.q118","maize.owner.agree.inspection.q119"
                                 ,"index_overall_off_baseF","maize.owner.agree.q96")
...
save(df_ols_D_sec_off,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_sec_off.Rdata",sep="/"))
...
baseline_dealers$mid_origin <- baseline_dealers$origin_mid
...
#5. Overall index
variables_overall_bag_mid <- cbind(baseline_dealers$mid_reading,baseline_dealers$mid_visible_packdate,baseline_dealers$mid_shelflife_Caro,baseline_dealers$mid_lot,baseline_dealers$mid_origin) #x
#no baseline_dealers$mid_visible_expdate because all same value (1)
variables_overall_bag_base <- cbind(baseline_dealers$reading,baseline_dealers$visible_packdate,baseline_dealers$shelflife_Caro,baseline_dealers$lot,baseline_dealers$origin)
...
for (i in 1:length(results_dealer_sec_bag)){
  df_means_D_sec_bag[1,i] <- sum(baseline_dealers[results_dealer_sec_bag_base[i]], na.rm=T)/(nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_sec_bag_base[i]])))
  df_means_D_sec_bag[2,i] <- sqrt(var(baseline_dealers[results_dealer_sec_bag_base[i]], na.rm=T))
  df_means_D_sec_bag[3,i] <- nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_sec_bag[i]]))}

save(df_means_D_sec_bag,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_sec_bag.Rdata",sep="/"))
...
save(df_ols_D_sec_bag,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_sec_bag.Rdata",sep="/"))
...
save(df_means_D_sec_bag_B,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_sec_bag_B.Rdata",sep="/"))
...
save(df_ols_D_sec_bag_B,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_sec_bag_B.Rdata",sep="/"))
...
index_dealer_endchain_mid_Kling <- Kling_index(xmat=variables_dealer_endchain_mid,sgroup = baseline_dealers$training_control)
baseline_dealers$index_dealer_endchain_midT_Kling <- index_dealer_endchain_mid_Kling$index
...
variables_dealer_efforts_mid <- cbind(baseline_dealers$index_efforts_mid,
                                      ...
                                      baseline_dealers$index_practices_lab_mid,
                                      baseline_dealers$index_practices_cap_mid,
                                      baseline_dealers$index_overall_off_mid)
...
index_dealer_efforts_mid_Kling <- Kling_index(xmat=variables_dealer_efforts_mid)
baseline_dealers$index_dealer_efforts_mid_Kling <- index_dealer_efforts_mid_Kling$index
...
index_dealer_efforts_mid_Kling <- Kling_index(xmat=variables_dealer_efforts_mid,sgroup = baseline_dealers$training_control)
baseline_dealers$index_dealer_efforts_midT_Kling <- index_dealer_efforts_mid_Kling$index
...
index_dealer_endchain_mid_Kling <- Kling_index(xmat=variables_dealer_endchain_mid,sgroup = baseline_dealers$clearing_control)
baseline_dealers$index_dealer_endchain_midC_Kling <- index_dealer_endchain_mid_Kling$index
...
index_dealer_efforts_mid_Kling <- Kling_index(xmat=variables_dealer_efforts_mid,sgroup = baseline_dealers$clearing_control)
baseline_dealers$index_dealer_efforts_midC_Kling <- index_dealer_efforts_mid_Kling$index
...
results_dealer_sec_nobase <- c("index_motivation_midC"
                               ...
                               ,"index_quality_alternative_midC_Kling")
...
index_dealer_endchain_mid_Kling <- Kling_index(xmat=variables_dealer_endchain_mid,sgroup = baseline_dealers$farmer_control)
baseline_dealers$index_dealer_endchain_midF_Kling <- index_dealer_endchain_mid_Kling$index
...
results_dealer_sec_nobase <- c("index_motivation_midF","index_ratings_midF"
                               ...
                               ,"index_dealer_efforts_midF","index_dealer_endchain_midF_Kling")
...
df_means_D_sec_nobase_HET <- array(NA,dim=c(3,10))

for (i in 1:length(results_dealer_sec_nobase)){
  df_means_D_sec_nobase_HET[1,i] <- sum(baseline_dealers[results_dealer_sec_nobase[i]], na.rm=T)/(nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_sec_nobase[i]])))
  df_means_D_sec_nobase_HET[2,i] <- sqrt(var(baseline_dealers[results_dealer_sec_nobase[i]], na.rm=T))
  df_means_D_sec_nobase_HET[3,i] <- nrow(baseline_dealers)-sum(is.na(baseline_dealers[results_dealer_sec_nobase[i]]))}

save(df_means_D_sec_nobase_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_D_sec_nobase_HET.Rdata",sep="/"))

###
#2#
###

df_ols_D_sec_nobase_HET <- array(NA,dim=c(3,3,10))
...
for (i in 1:length(results_dealer_sec_nobase)){
  ...
  df_ols_D_sec_nobase_HET[1,1,i] <- coef_test(ols, vcov_cluster)$beta[2]
  df_ols_D_sec_nobase_HET[2,1,i] <- coef_test(ols, vcov_cluster)$SE[2]
  df_ols_D_sec_nobase_HET[3,1,i] <- coef_test(ols, vcov_cluster)$p_Satt[2]}
...
for (i in 1:length(results_dealer_sec_nobase)){
  ...
  df_ols_D_sec_nobase_HET[1,2,i] <- coef_test(ols, vcov_cluster)$beta[3]
  df_ols_D_sec_nobase_HET[2,2,i] <- coef_test(ols, vcov_cluster)$SE[3]
  df_ols_D_sec_nobase_HET[3,2,i] <- coef_test(ols, vcov_cluster)$p_Satt[3]}
...
for (i in 1:length(results_dealer_sec_nobase)){
  ...
  #farmer video treatment at village/shop level so no clustering needed
  df_ols_D_sec_nobase_HET[1,3,i] <- summary(ols)$coefficients[4,1]
  df_ols_D_sec_nobase_HET[2,3,i] <- summary(ols)$coefficients[4,2]
  df_ols_D_sec_nobase_HET[3,3,i] <- summary(ols)$coefficients[4,4]}

save(df_ols_D_sec_nobase_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_D_sec_nobase_HET.Rdata",sep="/"))
...
df_means_F_prim_HET <- array(NA,dim=c(3,8))

for (i in 1:length(results_farmer_prim)){
  df_means_F_prim_HET[1,i] <- sum(baseline_farmers[results_farmer_prim[i]], na.rm=T)/(nrow(baseline_farmers)-sum(is.na(baseline_farmers[results_farmer_prim[i]])))
  df_means_F_prim_HET[2,i] <- sqrt(var(baseline_farmers[results_farmer_prim[i]], na.rm=T))
  df_means_F_prim_HET[3,i] <- nrow(baseline_farmers)-sum(is.na(baseline_farmers[results_farmer_prim[i]]))-sum(is.na(baseline_farmers[results_farmer_prim_base[i]]))+sum(is.na(baseline_farmers[results_farmer_prim[i]])&is.na(baseline_farmers[results_farmer_prim_base[i]]))}

save(df_means_F_prim_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_F_prim_HET.Rdata",sep="/"))
...
df_ols_F_prim_HET <- array(NA,dim=c(3,3,11))
...
for (i in 1:length(results_farmer_prim)){
  ...
  df_ols_F_prim_HET[1,1,i] <- coef_test(ols, vcov_cluster)$beta[2]
  df_ols_F_prim_HET[2,1,i] <- coef_test(ols, vcov_cluster)$SE[2]
  df_ols_F_prim_HET[3,1,i] <- coef_test(ols, vcov_cluster)$p_Satt[2]}
...
for (i in 1:length(results_farmer_prim)){
  ...
  df_ols_F_prim_HET[1,2,i] <- coef_test(ols, vcov_cluster)$beta[3]
  df_ols_F_prim_HET[2,2,i] <- coef_test(ols, vcov_cluster)$SE[3]
  df_ols_F_prim_HET[3,2,i] <- coef_test(ols, vcov_cluster)$p_Satt[3]}
...
for (i in 1:length(results_farmer_prim)){
  ...
  #farmer video treatment at village/shop level so no clustering needed
  df_ols_F_prim_HET[1,3,i] <- coef_test(ols, vcov_cluster_shop)$beta[4]
  df_ols_F_prim_HET[2,3,i] <- coef_test(ols, vcov_cluster_shop)$SE[4]
  df_ols_F_prim_HET[3,3,i] <- coef_test(ols, vcov_cluster_shop)$p_Satt[4]}

save(df_ols_F_prim_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_F_prim_HET.Rdata",sep="/"))
...
save(df_means_F_sec_yieldetc,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_F_sec_yieldetc.Rdata",sep="/"))
...
save(df_ols_F_sec_yieldetc,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_F_sec_yieldetc.Rdata",sep="/"))
...
df_means_F_nobase_HET <- array(NA,dim=c(3,12))

for (i in 1:length(results_farmer_nobase)){
  df_means_F_nobase_HET[1,i] <- sum(baseline_farmers[results_farmer_nobase[i]], na.rm=T)/(nrow(baseline_farmers)-sum(is.na(baseline_farmers[results_farmer_nobase[i]])))
  df_means_F_nobase_HET[2,i] <- sqrt(var(baseline_farmers[results_farmer_nobase[i]], na.rm=T))
  df_means_F_nobase_HET[3,i] <- nrow(baseline_farmers)-sum(is.na(baseline_farmers[results_farmer_nobase[i]]))}

save(df_means_F_nobase_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_means_F_nobase_HET.Rdata",sep="/"))

###
#2#
###

df_ols_F_nobase_HET <- array(NA,dim=c(3,3,12))
...
for (i in 1:length(results_farmer_nobase)){
  ...
  df_ols_F_nobase_HET[1,1,i] <- coef_test(ols, vcov_cluster)$beta[2]
  df_ols_F_nobase_HET[2,1,i] <- coef_test(ols, vcov_cluster)$SE[2]
  df_ols_F_nobase_HET[3,1,i] <- coef_test(ols, vcov_cluster)$p_Satt[2]}
...
for (i in 1:length(results_farmer_nobase)){
  ...
  df_ols_F_nobase_HET[1,2,i] <- coef_test(ols, vcov_cluster)$beta[3]
  df_ols_F_nobase_HET[2,2,i] <- coef_test(ols, vcov_cluster)$SE[3]
  df_ols_F_nobase_HET[3,2,i] <- coef_test(ols, vcov_cluster)$p_Satt[3]}
...
for (i in 1:length(results_farmer_nobase)){
  ...
  #farmer video treatment at village/shop level so no clustering needed
  df_ols_F_nobase_HET[1,3,i] <- coef_test(ols, vcov_cluster_shop)$beta[4]
  df_ols_F_nobase_HET[2,3,i] <- coef_test(ols, vcov_cluster_shop)$SE[4]
  df_ols_F_nobase_HET[3,3,i] <- coef_test(ols, vcov_cluster_shop)$p_Satt[4]}

save(df_ols_F_nobase_HET,file=paste(path,"papers/clearinghouse_training_paper/output_CH_training/df_ols_F_nobase_HET.Rdata",sep="/"))
