...
stack_1 <- baseline_farmers[c("farmer_ID","adoption_any")]
stack_1$adoption_any <- stack_1$adoption_any=="Yes"
stack_1$round <- "baseline" 
stack_2 <- midline_farmers[c("farmer_ID","mid_adoption_any")]
stack_2$mid_adoption_any <- stack_2$mid_adoption_any=="Yes"
stack_2$round <- "midline" 
names(stack_2) <- c("farmer_ID","adoption_any","round")
stack_3 <- endline_farmers[c("farmer_ID","end_adoption_any")]
stack_3$end_adoption_any <- stack_3$end_adoption_any=="Yes"
stack_3$round <- "endline" 
names(stack_3) <- c("farmer_ID","adoption_any","round")

alluvial_frame <- rbind(stack_1, stack_2, stack_3)
alluvial_frame <- subset(alluvial_frame, !is.na(alluvial_frame$adoption_any))
...
all$adoption_any <- all$adoption_any == "Yes"
all$mid_adoption_any <- all$mid_adoption_any =="Yes" 
all$end_adoption_any <- all$end_adoption_any =="Yes"

prop.table(table(alluvial_frame$adoption_any[alluvial_frame$round=='baseline']))
prop.table(table(alluvial_frame$adoption_any[alluvial_frame$round=='midline']))
prop.table(table(alluvial_frame$adoption_any[alluvial_frame$round=='endline']))

prop.table(table(all$adoption_any ==1))
prop.table(table(all$adoption_any ==1 & all$mid_adoption_any==0))
prop.table(table(all$adoption_any ==1 & all$mid_adoption_any==1  & all$end_adoption_any==0))

prop.table(table(all$adoption_any ==0))
prop.table(table(all$adoption_any ==0 & all$mid_adoption_any==1))
prop.table(table(all$adoption_any ==0 & all$mid_adoption_any==0  & all$end_adoption_any==1))

prop.table(table(all$adoption_any ==1 & all$mid_adoption_any==0  & all$end_adoption_any==1))

prop.table(table(all$adoption_any ==0 & all$mid_adoption_any==1  & all$end_adoption_any==0))
...
#here2
all <- merge(merge(baseline_farmers[c("farmer_ID","shop_ID","treatment","clearing","training","adoption_any", "Land_Races","farmer_saved_seed","Bought_from_agro_input_shop","Check2.check.maize.q9")],midline_farmers[c("farmer_ID","mid_adoption_any","mid_Land_Races","mid_farmer_saved_seed","mid_Bought_from_agro_input_shop")]),endline_farmers[c("farmer_ID","end_adoption_any","end_Land_Races", "end_farmer_saved_seed","end_Bought_from_agro_input_shop")])
all$adoption_any <- all$adoption_any == "Yes"
all$mid_adoption_any <- all$mid_adoption_any == "Yes"
all$end_adoption_any <- all$end_adoption_any == "Yes"
...
##  collect results
xmat_base <- cbind(all$adoption_any,all$farmer_saved_seed,all$Bought_from_agro_input_shop)
...
outcomes <- c("adoption_any","Land_Races","farmer_saved_seed","Bought_from_agro_input_shop","index_base" )
...
xmat_mid <- cbind(all$mid_adoption_any,all$mid_farmer_saved_seed,all$mid_Bought_from_agro_input_shop)
index_mid <- icwIndex(xmat=xmat_mid,revcols = c(2))
all$index_mid <- index_mid$index

mid_adoption <-  array(NA,dim=c(4,2,5)) #here
#loop here over outcomes c("mid_adoption_any","mid_Land_Races","mid_Bought_from_agro_input_shop")
outcomes <- c("mid_adoption_any","mid_Land_Races","mid_farmer_saved_seed", "mid_Bought_from_agro_input_shop","index_mid")
outcomes_base <- c("adoption_any","Land_Races","farmer_saved_seed","Bought_from_agro_input_shop","index_base" )
for (i in 1:length(outcomes)) {
...
ols <- lm(as.formula(paste(outcomes[i],paste("treatment*clearing*training",outcomes_base[i],sep="+"), sep="~")),data=all[all$adoption_any==1,])
vcov_cluster <- vcovCR(ols,cluster=all[all$adoption_any==1,]$shop_ID,type="CR0")
coef_test(ols, vcov_cluster)
...
sum(all$Check2.check.maize.q9>=0.7471055 & !is.na(all$mid_adoption_any),na.rm=T)
sum(all$Check2.check.maize.q9>=0.7471055 & !is.na(all$mid_adoption_any) & all$adoption_any==1,na.rm=T)
...
xmat_end <- cbind(all$end_adoption_any,all$end_farmer_saved_seed,all$end_Bought_from_agro_input_shop)
index_end <- icwIndex(xmat=xmat_end,revcols = c(2))
all$index_end <- index_end$index

end_adoption <-  array(NA,dim=c(4,2,5))
#loop here over outcomes c("mid_adoption_any","mid_Land_Races","mid_farmer_saved_seed","end_Bought_from_agro_input_shop")
outcomes <- c("end_adoption_any","end_Land_Races","end_farmer_saved_seed","end_Bought_from_agro_input_shop","index_end")
outcomes_base <- c("adoption_any","Land_Races","farmer_saved_seed","Bought_from_agro_input_shop","index_base" )
for (i in 1:length(outcomes)) {
...
all <- merge(baseline_farmers[c("farmer_ID","shop_ID","treatment","clearing","training","adoption_any")],endline_farmers[c("farmer_ID","q58_correct","q59_correct","q60_correct","q61_correct","q62_correct","q63_correct")], by="farmer_ID")
all$adoption_any <- all$adoption_any == "Yes"
all$mid_adoption_any <- all$mid_adoption_any == "Yes"
all$end_adoption_any <- all$end_adoption_any == "Yes"
...
for (i in 1:length(outcomes)) {
  ...
  ols <- lm(as.formula(paste(outcomes[i],"treatment*clearing*training", sep="~")),data=all[all$adoption_any==1,])
  vcov_cluster <- vcovCR(ols,cluster=all[all$adoption_any==1,]$shop_ID,type="CR0")
  coef_test(ols, vcov_cluster)
...
all <- merge(merge(baseline_farmers[c("farmer_ID","shop_ID","treatment","clearing","training","adoption_any", "correct_spacing", "correct_seed_rate", "organic_use", "DAP_use", "Urea_use", "times_weeding", "pesticide_use", "resowing","time_plant","time_weed")],midline_farmers[c("farmer_ID","mid_adoption_any","mid_correct_spacing", "mid_correct_seed_rate", "mid_organic_use", "mid_DAP_use", "mid_Urea_use", "mid_times_weeding", "mid_pesticide_use", "mid_resowing","mid_time_plant","mid_time_weed")]),endline_farmers[c("farmer_ID","end_adoption_any","end_correct_spacing", "end_correct_seed_rate", "end_organic_use", "end_DAP_use", "end_Urea_use", "end_times_weeding", "end_pesticide_use", "end_resowing","end_time_plant","end_time_weed")])
all$adoption_any <- all$adoption_any == "Yes"
all$mid_adoption_any <- all$mid_adoption_any == "Yes"
all$end_adoption_any <- all$end_adoption_any == "Yes"
...
#loop here over outcomes c("mid_adoption_any","mid_Land_Races","mid_farmer_saved_seed","mid_Bought_from_agro_input_shop")
...
for (i in 1:length(outcomes)) {
  ...
  ols <- lm(as.formula(paste(outcomes[i],paste("treatment*clearing*training",outcomes_base[i],sep="+"), sep="~")),data=all[all$adoption_any==1 & all$mid_adoption_any==0 &  all$end_adoption_any==1 ,])
  vcov_cluster <- vcovCR(ols,cluster=all[all$adoption_any==1 & all$mid_adoption_any==0 &  all$end_adoption_any==1,]$shop_ID,type="CR0")
  coef_test(ols, vcov_cluster) 
...
all <- merge(merge(baseline_farmers[c("farmer_ID","shop_ID","treatment","clearing","training","adoption_any","expectations","yield_inkg","landproductivity" )],midline_farmers[c("farmer_ID","mid_expectations_met","mid_myownfault","mid_yield_inkg","mid_landproductivity")]),endline_farmers[c("farmer_ID","end_expectations_met","end_myownfault","end_yield_inkg","end_landproductivity")])
all$adoption_any <- all$adoption_any == "Yes"
all$mid_adoption_any <- all$mid_adoption_any == "Yes"
all$end_adoption_any <- all$end_adoption_any == "Yes"
##demean orthogonal treatments
...
#
all <- merge(merge(baseline_farmers[c("farmer_ID","shop_ID","treatment","clearing","training","adoption_any","Check2.check.maize.q36","Check2.check.maize.q37" )],midline_farmers[c("farmer_ID","mid_adoption_any")]),endline_farmers[c("farmer_ID","end_expectations_met","end_myownfault","end_yield_inkg","end_landproductivity")])
##demean orthogonal treatments
