...
moisture <- read.csv(paste(path,"Moisture_Midline_2022_02_28_00_33_25_696986.csv", sep="/"))


moisture$age <- difftime(strptime("01.03.2022", format = "%d.%m.%Y"),strptime(moisture$date_pack,format="%Y-%m-%d"),units="days")
moisture$age[moisture$date_pack=="n/a"] <- difftime(strptime("01.03.2022", format = "%d.%m.%Y"),strptime(moisture$exp[moisture$date_pack=="n/a"] ,format="%Y-%m-%d"),units="days")+180
...
moisture$barcode[is.na(moisture$barcode) & moisture$id=="AD 24332"] <- 21
moisture$barcode[is.na(moisture$barcode) & moisture$id=="AD  17232"] <- 30
moisture$barcode[is.na(moisture$barcode) & moisture$id=="AD 8426"] <- 251
...
#just to try
summary(lm(reading~clearing, data=shops))
