...
rating_dyads <- read.csv(paste(path,"farmer/public/midline_rating_dyads.csv", sep="/"), stringsAsFactors = FALSE)
...
#write.csv(reviews, paste(path, "agro_input/raw/shiny_app/midline_reviews_general.csv",sep="/"), row.names=FALSE)
...
path_base <- strsplit(path, "/midline/data")[[1]]
